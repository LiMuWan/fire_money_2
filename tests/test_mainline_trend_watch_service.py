...
class MainlineTrendWatchServiceTest(unittest.TestCase):
    def test_build_report_explains_mainline_root_and_entry_plan(self) -> None:
        ...
        self.assertIn(item.strategy_type, {"龙头主升候选", "趋势主升候选", "低位主升预备"})
        self.assertIn("产业逻辑", item.strategy_fit)
        self.assertIn("净利增速", item.value_case)
        self.assertIn("成交额", item.capital_case)
        self.assertIn("产业链", item.industry_chain_case)
        self.assertIn("经济利益", item.profit_driver_case)
        self.assertIn("启动前", item.pre_breakout_case)
        self.assertIn("做T", item.t_plan)
        self.assertIn("回撤控制", item.risk_control_case)
        self.assertIn("真正值得盯", item.why_watch_case)
        self.assertTrue(
            any(token in item.main_wave_stage for token in ("主升", "趋势", "蓄势", "研究", "风险"))
        )
        self.assertIn("缩量回踩", item.confirmation_case)
        self.assertIn("10日线", item.holding_plan)
        self.assertIn("跌破", item.failure_signal)
        self.assertTrue(any(token in item.position_plan for token in ("小仓", "分批", "观察仓", "底仓", "只研究")))
