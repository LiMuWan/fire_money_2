...
class MainChainSmokeTest(unittest.TestCase):
    ...
    def test_candidate_and_position_include_exit_plan_and_continuity(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.assertEqual(candidate.exit_plan.strong_take_profit_pct, 0.095)
            self.assertGreater(candidate.exit_plan.stop_loss_pct, 0)
            self.assertLessEqual(candidate.exit_plan.stop_loss_pct, 0.0425)
            self.assertEqual(candidate.exit_plan.trailing_stop_pct, 0.01)
            self.assertIn("盈利 8%", candidate.exit_plan.summary)
            self.assertIn("9.5%", candidate.exit_plan.summary)
            self.assertIn("1%", candidate.exit_plan.summary)
    ...
    def test_one_to_two_config_is_json_and_defaults_to_single_core_line(self) -> None:
        ...
        self.assertEqual(settings.strong_take_profit_pct, 0.095)
        self.assertEqual(settings.stop_loss_pct, 0.0425)
        self.assertEqual(settings.trailing_stop_pct, 0.01)
